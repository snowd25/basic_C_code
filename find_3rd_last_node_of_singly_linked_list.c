...
struct node* find_third_last_node_of_linked_list(struct node *head)
{
	...
	int count = 0;
        ...
        while(NULL != fast_ptr && NULL != fast_ptr->next->next->next)
        {
		count += 1;
...
int main()
{
	...
	struct node* third_last = find_third_last_node_of_linked_list(head);
